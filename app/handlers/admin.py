...
async def process_add_admin_details(message: Message, storage_service: StorageService, loc_path: Path):
    ...
    await FSMContext(storage=router.storage, key=message.from_user.id, bot=message.bot).clear()
...
async def process_remove_admin_id(message: Message, storage_service: StorageService, loc_path: Path):
